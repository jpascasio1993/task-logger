...
class DashboardTaskBloc
  ...
  DashboardTaskBloc(this._getTasks, this._deleteTasks, this._watchTasks,
      this._syncLocalTasks, this._syncRemoteTasks)
    ...
    on<DashboardTaskEventWatchTasksUpdateTaskList>(_onUpdateTaskList,
        transformer: debounceDroppable(const Duration(milliseconds: 350)));
    on<DashboardTaskEventSyncRemoteTasks>(_onSyncRemoteTasks,
        transformer: debounceDroppable(const Duration(milliseconds: 350)));
  }
  ...
  final SyncRemoteTasks _syncRemoteTasks;
  ...
  Future<void> _onSyncRemoteTasks(DashboardTaskEventSyncRemoteTasks event,
      Emitter<DashboardTaskState> emit) async {
    await super.run(() async {
      final res = await _syncRemoteTasks();
      if (res is ResultError<bool>) {
        ...
        onProcess: (isLoading, exception) => emit(
              state.copyWith(
                  syncRemoteTasksLoading: isLoading,
                  syncRemoteTasksException: exception),
            ),
